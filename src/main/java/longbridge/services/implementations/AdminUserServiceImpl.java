...
/**
 * Created by SYLVESTER on 3/30/2017. Modified by Fortune
 */
@Service
	...
	public void createUserOnEntrust(AdminUser adminUser) {
		...
		if (setting != null && setting.isEnabled()) {
			if ("YES".equalsIgnoreCase(setting.getValue())) {
				...
				boolean contactResult = securityService.addUserContacts(adminUser.getEmail(),
						adminUser.getPhoneNumber(), true, adminUser.getUserName());
				if (!contactResult) {
		...
		try {
			...
			if ((oldStatus == null)) {
				...
				Email email = new Email.Builder().setRecipient(user.getEmail())
						...
						.setBody(String.format(messageSource.getMessage("admin.create.message", null, locale), fullName,
	...
	public String deleteUser(Long id) throws InternetBankingException {
		try {
			...
			throw new InternetBankingSecurityException(
					messageSource.getMessage("entrust.delete.failure", null, locale));
		} catch (Exception e) {
	...
	public String resetPassword(Long userId) throws PasswordException {
		...
		try {
			...
			Email email = new Email.Builder().setRecipient(user.getEmail())
					...
					.setBody(String.format(messageSource.getMessage("admin.password.reset.message", null, locale),
