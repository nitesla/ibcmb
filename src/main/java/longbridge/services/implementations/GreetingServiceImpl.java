...
public class GreetingServiceImpl implements GreetingService {
    ...
    public String addGreeting(GreetingDTO greetingDTO) {
        try {
            ...
            logger.warn("Verification Interrupted {}", e.getMessage());
            return e.getMessage();
        } catch (Exception e) {
            logger.error("greeting add failure {}", e.getMessage());
    ...
    public String updateGreeting(GreetingDTO dto) {
        try {
            ...
            logger.error("Could not add greeting. {} ",e.getMessage());
    ...
    public String deleteGreeting(Long id) {
        try {
            ...
            logger.debug("deleting {}",greeting);
            return messageSource.getMessage("greeting.delete.success", null, locale);
        } catch (VerificationInterruptedException e) {
            logger.warn("Verification Interrupted deleting {} with  {}",id, e.getMessage());
            return e.getMessage();
        } catch (Exception e) {
            logger.error("Error deleting {} with  {}", id, e.getMessage());
    ...
    public List<GreetingDTO> getCurrentGreetingsForUser() {
        ...
        logger.trace("todayGreetings {}", allCurrentGreetings);
    ...
    private List<GreetingDTO> getGeneralGreetings() {
        ...
        try {
            ...
            logger.warn("General Greetings {}", e.getMessage());
        }
    ...
    private List<GreetingDTO> getPersonalGreetings() {
        ...
        try {
            ...
            List<Greeting> greetings = greetingRepo.findPersonalGreetings(DateUtil.getToday(),currentUserId );
            logger.debug("pgreeting {},{}", greetings, currentUserId);
            for (Greeting greeting : greetings) {
                GreetingDTO greetingDTO = modelMapper.map(greeting, GreetingDTO.class);
                greetingDTO.setMessage(setmessageParameters(greetingDTO, getCurrentUser()));
            ...
            logger.warn("Personal Greetings {}", e.getMessage());
        }
    ...
    private List<GreetingDTO> getAccountGreetings() {
        ...
        try {
            ...
            logger.warn("Account Anniversary greeting  {}", e.getMessage());
