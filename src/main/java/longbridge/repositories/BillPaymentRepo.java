...
public interface BillPaymentRepo extends CommonRepo<BillPayment,Long> {
    ...
    @Query("select r from BillPayment r where r.requestReference=:requestReference and (upper(r.responseDescription) like %:search% or concat(r.amount,'')  like %:search% or upper(r.createdOn) like %:search%)order by r.createdOn desc")
