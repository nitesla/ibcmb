...
public interface TransferRequestRepo extends CommonRepo<TransRequest, Long> {
    ...
    @Query("select r from TransRequest r where r.userReferenceNumber=:userReference and (upper(r.remarks) like %:search% or concat(r.amount,'') like %:search% or upper(r.beneficiaryAccountName) like %:search% or upper(r.tranDate) like %:search%)order by r.tranDate desc")
