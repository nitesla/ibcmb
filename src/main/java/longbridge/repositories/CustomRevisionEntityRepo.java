...
public interface CustomRevisionEntityRepo extends CommonRepo<CustomRevisionEntity,Long>
{
    ...
    @Query("select c.id from CustomRevisionEntity c where c.id in :revisionList")
    List<CustomRevisionEntity> findCustomEnityDetails(@Param("revisionList") List<Integer> revisionNumber);
